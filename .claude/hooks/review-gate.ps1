...
# (code-review/simplify) or generated by run-gates.sh (run-gates) — this gate proves a
# receipt exists for this exact code, not that the underlying check was done well, and
# NOT that it happened at all. An agent with shell access can call save-review-receipt.ps1
# -Kind run-gates by hand just as easily as it can type a fake code-review summary; the
# convention that run-gates.sh is the only intended writer is a norm this hook cannot
# enforce, not a technical guarantee. The non-forgeable enforcement for all three receipts
# is CI (correctness-review, security-review, grader, OWASP), which re-derives its verdict
