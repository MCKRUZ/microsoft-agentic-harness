...
#
# IMPORTANT — trust boundary differs by receipt. For code-review/simplify, a receipt's
# CONTENT is whatever was piped into the writer — this gate proves a receipt exists for
# this exact code, not that the review was done well. The non-forgeable enforcement for
# those two is CI (correctness-review, security-review, grader, OWASP), which re-derives
# its verdict server-side. The run-gates receipt is different: it is written by the
# deterministic script itself only on a real, full, passing local run — the agent cannot
# fabricate one by typing a summary, so its mere existence is stronger evidence than the
# other two.
#
...
if ($missing.Count -gt 0) {
  ...
  $parts = @()
  if ($skillSteps.Count -gt 0) {
    $parts += "run the missing skill(s) [$($skillSteps -join ', ')], then record each by piping " +
      "its summary to .claude/hooks/save-review-receipt.ps1 (e.g. ``'...summary...' | pwsh " +
