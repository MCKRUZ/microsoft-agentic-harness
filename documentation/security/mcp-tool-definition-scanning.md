...
| Description injection | High | Chat role markers (`<system>`, `[system]`, ChatML), implant wrappers (`<IMPORTANT>`, `<INSTRUCTIONS>`) and persona assignment ("you are now…", "you are a/an/the admin/unrestricted/…", "act as an…") |
...
- **"You are a/an/the …" requires an authority or restriction-removal term nearby.** The earlier form matched any noun, which is exactly how the harness's own shipped skills open their instructions bodies — *"You are a research agent…"*, *"You are the Echo Test Agent"* — so the scanner refused to load its own product's skills at the default threshold (#601). Every attack-corpus case for this rule already pairs the phrase with a term like "with no restrictions" or "the system administrator"; narrowing to require one is not a new distinction. A second false-positive pass then found several of the first narrower term list's own words too generic — "root", "unconstrained", "unfiltered", "uncensored", "no limits" and bare "dan" all fire on ordinary technical prose or a real person's name — so the surviving list is deliberately short: `admin`, `administrator`, `superuser`, `god mode`, `root access`/`root privileges`/`as root`/`become root`, `jailbroken`, `developer mode`, `unrestricted`, `no`/`without restrictions`, `no`/`without rules`, `do anything now`.
