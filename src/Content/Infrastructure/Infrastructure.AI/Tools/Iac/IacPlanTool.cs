...
public sealed class IacPlanTool : ITool
{
    ...
    /// <summary>
    /// The sandbox capabilities a plan run needs: reads and writes the module directory (terraform's
    /// <c>.terraform</c> cache and plan file), spawns the CLI as a subprocess, and reaches the
    /// provider/module registries. The single source of truth <c>TerraformGenerator</c>/
    /// <c>BicepGenerator</c> pass into <see cref="Infrastructure.AI.Iac.IacSandboxRunner.RunAsync"/>
    /// for every <c>iac_plan</c> call — no longer a hardcoded literal inside the runner (#387).
    /// </summary>
