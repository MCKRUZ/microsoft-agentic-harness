...
/// host registered <see cref="AgentRegistryApiMarker"/> via
...
/// routes exist. Mirrors <c>RequiresDriftApiOptInAttribute</c>.
/// </remarks>
[AttributeUsage(AttributeTargets.Class)]
public sealed class RequiresAgentRegistryApiOptInAttribute : Attribute, IActionConstraint
{
    /// <inheritdoc />
    public int Order => 0;

    /// <summary>Accepts the candidate action only when the host opted into the agent registry API.</summary>
    /// <param name="context">The constraint context supplied by routing.</param>
    /// <returns><see langword="true"/> when <see cref="AgentRegistryApiMarker"/> is registered.</returns>
    public bool Accept(ActionConstraintContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        return context.RouteContext.HttpContext.RequestServices
            .GetService(typeof(AgentRegistryApiMarker)) is not null;
    }
}
