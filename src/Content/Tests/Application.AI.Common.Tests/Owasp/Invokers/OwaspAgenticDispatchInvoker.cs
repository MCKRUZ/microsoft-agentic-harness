...
/// <summary>
/// Routes each OWASP Agentic eval case to its dedicated invoker by <see cref="EvalCase.Id"/>.
/// Most cases use deterministic stub invokers; <c>asi06_memory_poison</c> uses
/// <see cref="OwaspAsi06RuntimeInvoker"/>, which drives the real memory quarantine runtime.
...
public sealed class OwaspAgenticDispatchInvoker : IAgentInvoker
{
    private static readonly IReadOnlyDictionary<string, IAgentInvoker> Invokers =
        new Dictionary<string, IAgentInvoker>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["asi06_memory_poison"] = new OwaspAsi06RuntimeInvoker(),
        ...
        CancellationToken cancellationToken)
    {
        if (!Invokers.TryGetValue(@case.Id, out var invoker))
            throw new InvalidOperationException(
                $"No invoker registered for OWASP eval case '{@case.Id}'. " +
