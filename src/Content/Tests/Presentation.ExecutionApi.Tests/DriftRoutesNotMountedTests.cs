...
/// The execution API references <c>Presentation.Common</c> and calls <c>AddControllers()</c>, so the Web
/// SDK auto-registers Presentation.Common as an MVC application part and
/// <c>DriftController</c> IS discovered here — but the execution API never calls <c>AddDriftApi()</c>,
