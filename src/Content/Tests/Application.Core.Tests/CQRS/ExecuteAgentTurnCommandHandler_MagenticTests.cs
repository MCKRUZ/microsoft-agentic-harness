...
public sealed class ExecuteAgentTurnCommandHandler_MagenticTests
{
    ...
    public async Task Handle_AgentIsMagenticSupervisor_DelegatesToMagenticTurnRunner()
    {
        ...
        _magenticTurnRunner
            .Setup(r => r.RunTurnAsync(supervisor, "Plan and execute the task", It.IsAny<IReadOnlyList<ChatMessage>>(), It.IsAny<CancellationToken>()))
        ...
        _magenticTurnRunner.Verify(
            r => r.RunTurnAsync(supervisor, "Plan and execute the task", It.IsAny<IReadOnlyList<ChatMessage>>(), It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task Handle_MagenticTurnFails_ReturnsFailureFromRunnerUnmodified()
    {
        ...
        _magenticTurnRunner
            .Setup(r => r.RunTurnAsync(supervisor, It.IsAny<string>(), It.IsAny<IReadOnlyList<ChatMessage>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new AgentTurnResult
            {
                Success = false,
    ...
    public async Task Handle_AgentIsOrdinarySingleAgent_NeverCallsMagenticTurnRunner()
    {
        ...
        _magenticTurnRunner.Verify(
            r => r.RunTurnAsync(
                It.IsAny<AgentDefinition>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<ChatMessage>>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
}
