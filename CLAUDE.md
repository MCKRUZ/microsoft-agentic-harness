...
- **Discriminating an outcome by a `SandboxExecutionResult` field that carries a different meaning depending on which code path produced it.** `ExitCode is null` looks like a clean signal for "the sandbox refused to dispatch this call before it ever ran" (a governance denial), but `ProcessSandboxExecutor`/`DockerSandboxExecutor` also leave `ExitCode` null on a timeout, a reserved-env-grant rejection, and an egress-preflight block — every one of which genuinely dispatched and genuinely signed a failure attestation. A first cut of the #405/#406/#407 governance-denial fix (`TerraformGenerator`/`BicepGenerator`) used `ExitCode is null` and silently mislabeled all three of those as governance denials; caught only on a second review pass. `Attestation is null` is the one field both executors leave unset in exactly the single case that matters: the caller refused the tool before reaching an executor at all, because attestation signing happens inside the executor. When adding a new discriminator over a shared result type, check what every producer of that type actually sets on every path, not just the two cases in front of you.
